...
## P0: CRM + 카카오 발송 안정화
- [x] 에이전트 V4.1 로직(Heartbeat, Recovery, Retry) 로컬 검증 및 빌드 테스트 완료
- [ ] 다중 Agent 스트레스 테스트 (Atomic Lock 동시성 검증)
- [ ] pytest 기반 단위 테스트 및 API 통합 테스트 코드 작성
- [ ] 카카오톡 GUI 클립보드 경합 문제 회피 최적화
- [ ] 카카오톡 버전별 호환성 테스트 및 DPI 125% / 150% 편차 검증

## P1: 예약 발송 무인 자동화 + 운영 안정성
- [ ] Agent 상시 실행 모드 개발 (Background Agent)
- [ ] Windows 시작 시 자동 실행 및 Windows Service 지원
- [ ] Tray Agent 아이콘 모드 지원
- [ ] 예약시간 자동 감시 및 사용자 개입 없는 완전 무인 예약 발송 처리
- [ ] 발송 상태(대기/진행/완료/에러)별로 Agent UI 내역 색상 구분
