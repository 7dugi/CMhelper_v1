...
### 2026-08-26 | Agent: pywinauto UIA 어댑터 도입 및 Fail-Closed 채팅 탐색/정리 체계
- **주제:** 카카오톡 고정 좌표 클릭 및 단축키 기반 채팅 진입 방식의 UIA 어댑터 전환
- **선택:**
  - `CMhelper_agent/kakao_ui.py`를 신설하여 `pywinauto` UIA 기반으로 채팅 탭 탐색, 검색, 1건 결과 진입, 창 제목 검증, 임시 검색 정리를 전담.
  - 명시적 바운디드 상수(5.0초 타임아웃, 최대 3회 재시도, 0.5초 간격)를 정의하여 무한 대기 차단.
  - 접근 가능한 이름 기반 채팅 탭("채팅", "Chats") 전용 선택 및 상태 검증.
  - 친구 탭, 친구 추가, 새로운 채팅, 고정 좌표 클릭, 이미지 매칭 자동화, `Ctrl+2` 단축키 사용을 엄격히 배제.
  - 검색 결과 단일 일치(1건) 검증: 0건 또는 2건 이상(동명이인/중복), 팝업 감지, 타임아웃, 창 제목 불일치 시 즉시 Fail-Closed로 발송을 차단하고 대기열 항목을 실패 처리.
  - 모든 탐색 시도(정상 완료, 안전 실패, 취소, 예외) 후 `finally` 블록에서 멱등한 `cleanup_search()`를 수행하여 임시 검색어 및 상태를 복구(카카오 데이터 보존).
  - 자동화 단위 테스트는 100% 가짜 UI 객체만 사용하여 검증하며 실제 메시지 발송은 배제.
  - 단순 git revert를 통해 즉시 롤백 가능하도록(Rollback-by-Revert) 아키텍처 격리.
- **배제:** 친구 목록 탐색/친구 추가 폴백, `Ctrl+2` 단축키, 맹목적 좌표/이미지 매칭 폴백, 자동 테스트 중 실제 메시지 발송.
- **결정 이유:** 해상도/창 크기/다크모드 변경에 따른 오작동 및 오발송(동명이인/엉뚱한 대화방 진입) 리스크를 완전히 차단하고 B2B SaaS 수준의 안정성을 확보.
- **상태:** 구현 및 검증 완료 [IMPLEMENTED & VERIFIED]
